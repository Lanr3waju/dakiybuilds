  ...
  return (
    <section className="m-6 rounded-tr-2xl bg-slate-50 p-4 shadow-md shadow-base-300">
      <header className="flex justify-between">
        <h2 className="w-full font-Poppins text-lg font-semibold uppercase text-primary">
          {log?.title}
        </h2>
        <button
          disabled={updateLog}
          onClick={editLog}
          className="btn-primary btn-xs mb-1 p-1 duration-200 active:translate-y-[2px] disabled:translate-y-0 disabled:cursor-not-allowed"
        >
          <EditIcon className="text-primary-content" />
        </button>
      </header>
      <HorizontalLine />
      {updateLog ? (
        ...
        <p className="mb-2 mt-3 leading-9 tracking-wide underline underline-offset-4">
          {log?.note}
        </p>
      )}
      <p className="mt-5 font-Poppins text-base leading-5 tracking-wide text-primary-content">
