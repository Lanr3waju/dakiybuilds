...
function OrganizationForm({ setActivateUserForm, activateUserForm }) {
  ...
  const validateForm = () => {
    ...
    if (!organizationFormData.organizationTel) {
      ...
      errors.organizationTel =
        'Invalid telephone format, should be 11 digits e.g. (08051551565)'
    }
  ...
  const handleSubmission = async (event) => {
    const { organizationEmail, organizationName, organizationTel, staffs } =
      organizationFormData
    event.preventDefault()
    if (validateForm()) {
      setIsLoading(true)
      const error = await organizationTable(
        organizationName,
        organizationEmail,
        organizationTel,
        staffs
      ...
      if (!errorMessage) {
        ...
        alert(
          'Organization Data successfully saved, Please fill in the next form with your profile data!'
        )
      } else {
        setIsLoading(false)
  ...
  return (
    <section className={`${activateUserForm ? 'hidden' : 'block'}`}>
      <div className="flex flex-col items-end justify-between md:flex-row">
        <h2 className="my-3 font-bold uppercase text-primary md:text-lg">
