...
function JobDetailsComponent() {
    ...
    const { projects, projectSumAndDate, setCurrentProjectId, project } = useContext(DakiyStore)
    ...
    const getProjectUpdates = async (id, initial_advance_payment) => {
        ...
        if (results) {
            ...
            setContractPayments(Number(totalContractPayments) + Number(initial_advance_payment))
        }
    }
    ...
    useEffect(() => {
        ...
        if (selectedProject) {
            ...
            getProjectUpdates(selectedProject.id, selectedProject.initial_advance_payment)
        }
    ...
    return (
        <>
            ...
            {editState ?
                (<EditJobState currentProject={currentProject} contractPayments={contractPayments} setEditState={setEditState} />)
                :
                (
                    <>
                        <header>
                        ...
                        <section className="flex flex-col items-start justify-between p-4 font-Raleway font-medium text-primary-content/75 md:flex-row">
                            <section className="mb-6 mr-1 w-11/12 rounded-md border-4 border-base-300 p-6 pb-10 shadow-md shadow-base-300" >
                                ...
                                <p className="font-Roboto text-lg uppercase"><span className="m-1 mt-4 block font-Raleway text-sm text-secondary-content/70">Balance Due to Contractor:</span>₦{addCommasToMoney((projectSumAndDate.projectContractSum - contractPayments))}</p>
                                ...
                                <p className="font-Roboto text-lg uppercase"><span className="m-1 mt-4 block font-Raleway text-sm text-secondary-content/70">Total contract payments:</span>₦ {addCommasToMoney(contractPayments)} Naira</p>
                            ...
                            <section className="ml-1 w-11/12 rounded-md border-4 border-base-300 p-4 pb-5 shadow-md shadow-base-300">
                                <p className="mb-3 text-lg uppercase"><span className="m-1 text-center font-Roboto text-sm text-secondary-content/70">Project Type:</span>{currentProject.type}</p>
                                ...
                                <Divider sx={{ my: 3 }} />
                                <p className="mb-3 font-Roboto text-lg font-bold uppercase tracking-wider"><span className="m-1 text-sm text-secondary-content/70">Client Name:</span>{currentProject.client_name}</p>
                                <p className="mb-3 font-Roboto text-lg font-semibold uppercase"><span className="m-1 text-sm text-secondary-content/70">Client Email:</span> {currentProject.client_email}</p>
                                <p className="mb-3 font-Roboto text-lg font-semibold uppercase"><span className="m-1 text-sm text-secondary-content/70">Client Tel:</span> {currentProject.client_phone}</p>
                                ...
                                <button onClick={() => setEditState(true)} className="btn btn-warning m-1 w-full">
                                    Update this project
                                </button>
                                {deleteState ?
