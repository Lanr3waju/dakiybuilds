...
function EditJobState({ currentProject, contractPayments, setEditState }) {
  ...
  const validateForm = () => {
    if (
      !formData.subsequentPayments &&
      !formData.newContractSum &&
      !formData.newFinishDate
    ) {
    ...
    if (
      formData.newFinishDate !== '' &&
      formData.newFinishDate < currentProject.start_date
    ) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      const insertProjectPlusTableResult = await insertProjectPlusTable(
        formData,
        currentProject
      )
      await updateProjectTable(formData, currentProject)
  ...
  return (
    <>
      <EditJobModal />
      <div
        className="mx-auto my-4 flex w-3/4 max-w-fit flex-col rounded-xl border-4 border-base-300 bg-base-200 p-4 font-Roboto"
        ref={containerRef}
      >
        <h2 className="mb-1 flex items-center justify-between text-2xl font-semibold">
          Update Job Details{' '}
          <button
            onClick={() => setEditState(false)}
            className="btn btn-square btn-error text-lg"
          >
            {' '}
            X
          </button>
        ...
        <p className="my-4 w-full rounded-md bg-info p-4 font-medium text-info-content">
          This form serves to capture new data or events related to the
          project&apos;s progression, rather than modify existing information.
        </p>
        <label className="mb-2 font-Roboto text-sm font-semibold tracking-widest text-primary-content/40">
          {' '}
          Enter new project finish date :
          <input
            type="date"
            name="newFinishDate"
        ...
        <label className="mb-2 font-Roboto text-sm font-semibold tracking-widest text-primary-content/40">
          {' '}
          Enter subsequent payments made by client:
          <input
            type="number"
